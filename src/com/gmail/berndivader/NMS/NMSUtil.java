...
public class NMSUtil {
	...
	static {
		...
		try {
			...
			class_World_explodeMethod = class_World.getMethod("createExplosion", class_Entity, Double.TYPE, Double.TYPE,
					Double.TYPE, Float.TYPE, Boolean.TYPE, Boolean.TYPE);
			...
			class_PacketPlayOutCustomSoundEffect_Constructor = class_PacketPlayOutCustomSoundEffect.getConstructor(
					String.class, class_EnumSoundCategory, Double.TYPE, Double.TYPE, Double.TYPE, Float.TYPE,
					Float.TYPE);
			...
			// We don't want to consider new-ish builds as "legacy" and print a
			// warning, so keep a separate flag
			...
			try {
				try {
					...
					try {
						// 1.10
						class_EntityArrow_lifeField = class_EntityArrow.getDeclaredField("ay"); // ayyyyy
																								// lmao
	...
	public static boolean createExplosion(Entity entity, World world, double x, double y, double z, float power,
		...
		try {
			...
			Object explosion = class_World_explodeMethod.invoke(worldHandle, entityHandle, x, y, z, power, setFire,
					breakBlocks);
	...
	public static Object getTileEntityData(Location location) {
		...
		try {
			...
			Object tileEntity = class_CraftWorld_getTileEntityAtMethod.invoke(world, location.getBlockX(),
	...
	public static void clearItems(Location location) {
		if (class_TileEntity_loadMethod == null || class_TileEntity_updateMethod == null
				|| class_CraftWorld_getTileEntityAtMethod == null || class_TileEntity_saveMethod == null)
			return;
		if (location == null)
			return;
		try {
	...
	public static void setTileEntityData(Location location, Object data) {
		if (class_TileEntity_loadMethod == null || class_TileEntity_updateMethod == null
				|| class_CraftWorld_getTileEntityAtMethod == null)
			return;
	...
	public static void playCustomSound(Player player, Location location, String sound, float volume, float pitch) {
		try {
			Object packet = class_PacketPlayOutCustomSoundEffect_Constructor.newInstance(sound,
					enum_SoundCategory_PLAYERS, location.getX(), location.getY(), location.getZ(), volume, pitch);
			sendPacket(player, packet);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
