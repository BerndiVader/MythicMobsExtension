...
public class mmStealSkill extends SkillMechanic implements ITargetedEntitySkill {
	...
	public boolean castAtEntity(SkillMetadata data, AbstractEntity target) {
        ...
        Inventory pit = pl.getInventory(); String ri,it; ItemStack item; int a,ra; boolean stolen = false;
        for (String ll : items) {
            ListIterator<ItemStack> ii = pit.iterator();
    		temp = ll.split(":"); ri = temp[0];
			try {ra = Integer.parseInt(temp[1]);} catch (NumberFormatException e) {ra = 1;}
			while (ii.hasNext() && !stolen) {item = ii.next();
				if (item!=null) { it = item.getData().getItemType().toString(); a = item.getAmount();
					if (ri.equalsIgnoreCase(it)) {
						if (ra < a) {ii.set(new ItemStack(item.getData().getItemType(),a-ra));}
						else {ra = a; ii.set(new ItemStack(Material.AIR));}
						am.signalMob(am.getEntity(), this.signal_ok);
						Main.thiefhandler().addThief(am.getUniqueId(), new ItemStack(item.getData().getItemType(),ra));
						stolen = true; break;
					}
				}
				if (stolen) {break;}
