...
ITargetedEntitySkill {
	...
	public void cancelInventoryClick(InventoryClickEvent e) {
		if(uuids.contains(e.getWhoClicked().getUniqueId())) e.setCancelled(true);
	}
	...
	Listener
	{
		...
		public StunTracker(AbstractLocation location,AbstractEntity target,long duration) {
			...
			if (final_target.hasMetadata(Utils.meta_STUNNED)) target.getBukkitEntity().removeMetadata(Utils.meta_STUNNED, Main.getPlugin());
			if(final_target.hasMetadata(Utils.meta_STUNNED+"Time")) target.getBukkitEntity().removeMetadata(Utils.meta_STUNNED+"Time", Main.getPlugin());
			...
			target.getBukkitEntity().setMetadata(Utils.meta_STUNNED+"Time", new FixedMetadataValue(Main.getPlugin(),duration));
			this.ai_status=final_target.hasAI();
			if(cancel_interaction) {
				if(uuids.contains(final_target.getUniqueId())) uuids.remove(final_target.getUniqueId());
		...
		public void run() {
			if (final_target==null
				...
				if(StunextMechanic.this.use_stun) {
					if (final_target.hasAI()&&final_target.isOnGround()) final_target.setAI(false);
					if (facing) {
						yaw=final_target.getLocation().getYaw();
						pitch=final_target.getLocation().getPitch();
					}
					if (gravity) y=final_target.getLocation().getY();
					Volatile.handler.forceSetPositionRotation(this.final_target,x,y,z,yaw,pitch,facing,gravity);
				}
				final_target.removeMetadata(Utils.meta_STUNNED+"Time",Main.getPlugin());;
				final_target.setMetadata(Utils.meta_STUNNED+"Time",new FixedMetadataValue(Main.getPlugin(),(int)this.duration-count));
			}
