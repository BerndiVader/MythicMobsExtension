...
IEntityCondition {
	...
	public SpawnReasonCondition(String line, MythicLineConfig mlc) {
		...
		for(String s1:mlc.getString(new String[]{"spawnreasons","spawnreason","reasons","reason","sr","r","types","type","t"},"NATURAL").toUpperCase().split(",")) {
		...
		if(custom=s1!=null) customs=s1.toUpperCase().split(",");
	}
	...
	public boolean check(AbstractEntity entity) {
		if(!custom) {
			...
			if(entity.getBukkitEntity().hasMetadata(Utils.meta_CUSTOMSPAWNREASON)) {
				...
				for(int i1=0;i1<reasons.length;i1++) {
					if(s1.equals(customs[i1])) return true;
				}
			}
		}
		return false;
