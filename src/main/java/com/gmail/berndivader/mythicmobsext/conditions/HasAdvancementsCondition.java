...
IEntityCondition {
	...
	public HasAdvancementsCondition(String line, MythicLineConfig mlc) {
		super(line,mlc);
		advs=new HashSet<>();
		String[]arr1=mlc.getString(new String[] {"advancement","adv","advancements"},"").toLowerCase().split(",");
		for(Iterator<Advancement>iter=Bukkit.getServer().advancementIterator();iter.hasNext();) {
			Advancement adv=iter.next();
			for(int i1=0;i1<arr1.length;i1++) {
				if (adv.getKey().getKey().toLowerCase().equals(arr1[i1])) advs.add(adv);
			}
		}
	}
	...
	public boolean check(AbstractEntity entity) {
		if (entity.isPlayer()) {
			...
			for(Advancement adv:advs) {
				if (p.getAdvancementProgress(adv).isDone()) return true;
			}
		}
		return false;
	}
