 ...
 * 
 * These are generally here when there is a new method added to the Bukkti API
 * we'd like to use, but aren't quite ready to give up backwards compatibility.
 * 
 * The easy solution to this problem is to shamelessly copy Bukkit's code in
 * here, mark it as deprecated and then switch everything over once the new
 * Bukkit method is in an official release.
 */
public class CompatibilityUtils extends NMSUtils {
			...
			if (currentEffect.getType().equals(effect.getType())) {
				if (effect.getAmplifier() < 0) {
					...
					break;
						...
						|| effect.getDuration() > Integer.MAX_VALUE / 4) {
					applyEffect = false;
					break;
				}
			}
	...
	public static void setSitting(Entity entity, boolean flag) {
		if (class_Sittable == null)
			return;
		if (!class_Sittable.isAssignableFrom(entity.getClass()))
			return;
		try {
	...
	public static Entity createEntity(Location location, EntityType entityType) {
		...
		try {
			...
			if (newEntity != null) {
				...
				if (bukkitEntity == null || !entityClass.isAssignableFrom(bukkitEntity.getClass()))
					return null;
			}
		} catch (Throwable ex) {
			ex.printStackTrace();
	...
	public static List<Entity> getNearbyEntities(Location location, double x, double y, double z) {
		...
		try {
			...
			Object bb = class_AxisAlignedBB_Constructor.newInstance(location.getX() - x, location.getY() - y,
					location.getZ() - z, location.getX() + x, location.getY() + y, location.getZ() + z);

			// The input entity is only used for equivalency testing, so this "null" should
			// be ok.
			@SuppressWarnings("unchecked")
			List<? extends Object> entityList = (List<? extends Object>) class_World_getEntitiesMethod
					.invoke(worldHandle, null, bb);
	...
	public static Minecart spawnCustomMinecart(Location location, Material material, short data, int offset) {
		...
		try {
			Constructor<?> minecartConstructor = class_EntityMinecartRideable.getConstructor(class_World, Double.TYPE,
					Double.TYPE, Double.TYPE);
			Method addEntity = class_World.getMethod("addEntity", class_Entity, CreatureSpawnEvent.SpawnReason.class);
			Method setPositionRotationMethod = class_Entity.getMethod("setPositionRotation", Double.TYPE, Double.TYPE,
					Double.TYPE, Float.TYPE, Float.TYPE);

			Object worldHandle = getHandle(location.getWorld());
			Object newEntity = minecartConstructor.newInstance(worldHandle, location.getX(), location.getY(),
					location.getZ());
			if (newEntity != null) {
				// Set initial rotation
				setPositionRotationMethod.invoke(newEntity, location.getX(), location.getY(), location.getZ(),
						location.getYaw(), location.getPitch());

				// Set tile material id, pack into NMS 3-byte format
				// TODO: Unbreak this maybe one day?
				/*
				 * int materialId = (display.getMaterial().getId() & 0xFFFF) |
				 * (display.getData() << 16); watch(newEntity, 20, materialId);
				 * 
				 * // Set the tile offset watch(newEntity, 21, offset);
				 * 
				 * // Finalize custom display tile watch(newEntity, 22, (byte)1);
				 */
	...
	public static ConfigurationSection loadConfiguration(String fileName)
	...
	public static boolean setLock(Block block, String lockName) {
		if (class_ChestLock_Constructor == null)
			return false;
		if (class_TileEntityContainer_setLock == null && class_TileEntityContainer_lock == null)
			return false;
	...
	public static boolean isLocked(Block block) {
		if (class_TileEntityContainer_getLock == null && class_TileEntityContainer_lock == null)
			return false;
		Object tileEntity = getTileEntity(block.getLocation());
		if (tileEntity == null)
			return false;
	...
	public static String getLock(Block block) {
		if (class_ChestLock_getString == null && class_ChestLock_key == null)
			return null;
		if (class_TileEntityContainer_getLock == null && class_TileEntityContainer_lock == null)
			return null;
		Object tileEntity = getTileEntity(block.getLocation());
	...
	public static boolean isValidProjectileClass(Class<?> projectileType) {
		return projectileType != null && (class_EntityArrow.isAssignableFrom(projectileType)
				|| class_EntityProjectile.isAssignableFrom(projectileType)
				|| class_EntityFireball.isAssignableFrom(projectileType));
	}

	public static Projectile spawnProjectile(Class<?> projectileType, Location location, Vector direction,
			ProjectileSource source, float speed, float spread, float spreadLocations, Random random) {
		...
		try {
			...
			if (class_EntityProjectile.isAssignableFrom(projectileType)
					|| class_EntityArrow.isAssignableFrom(projectileType)) {
				shootMethod = projectileType.getMethod("shoot", Double.TYPE, Double.TYPE, Double.TYPE, Float.TYPE,
						Float.TYPE);
			}

			setPositionRotationMethod = projectileType.getMethod("setPositionRotation", Double.TYPE, Double.TYPE,
					Double.TYPE, Float.TYPE, Float.TYPE);
			projectileSourceField = projectileType.getField("projectileSource");

			Object nmsProjectile = null;
			try {
				nmsProjectile = entityType == null ? constructor.newInstance(nmsWorld)
						: constructor.newInstance(entityType, nmsWorld);
			} catch (Exception ex) {
				nmsProjectile = null;
				Bukkit.getLogger().log(Level.WARNING, "Error spawning projectile of class " + projectileType.getName(),
						ex);
			}
			...
			// Velocity must be set manually- EntityFireball.setDirection applies a
			// crazy-wide gaussian distribution!
			...
			if (class_EntityFireball.isAssignableFrom(projectileType) && spreadLocations > 0) {
				modifiedLocation
						.setX(modifiedLocation.getX() + direction.getX() + (random.nextGaussian() * spread / 5));
				modifiedLocation
						.setY(modifiedLocation.getY() + direction.getY() + (random.nextGaussian() * spread / 5));
				modifiedLocation
						.setZ(modifiedLocation.getZ() + direction.getZ() + (random.nextGaussian() * spread / 5));
			}
			setPositionRotationMethod.invoke(nmsProjectile, modifiedLocation.getX(), modifiedLocation.getY(),
					modifiedLocation.getZ(), location.getYaw(), location.getPitch());
	...
	public static Entity spawnEntity(Location target, EntityType entityType,
		...
		try {
			...
			try {
				if (!class_CraftWorld_spawnMethod_isLegacy) {
					entity = (Entity) class_CraftWorld_spawnMethod.invoke(world, target, entityType.getEntityClass(),
							null, spawnReason);
				} else {
					entity = (Entity) class_CraftWorld_spawnMethod.invoke(world, target, entityType.getEntityClass(),
							spawnReason);
	...
	public static void sendExperienceUpdate(Player player, float experience, int level) {
		try {
			Object packet = class_PacketPlayOutExperience_Constructor.newInstance(experience,
	...
	public static void swingOffhand(Entity entity, int range) {
		...
		for (Player player : Bukkit.getOnlinePlayers()) {
			if (!player.getWorld().getName().equals(worldName)
					|| player.getLocation().distanceSquared(center) > rangeSquared) {
				continue;
			}
	...
	private static void sendBreaking(Player player, long id, Location location, int breakAmount) {
		try {
			Object blockPosition = class_BlockPosition_Constructor.newInstance(location.getX(), location.getY(),
					location.getZ());
			Object packet = class_PacketPlayOutBlockBreakAnimation_Constructor.newInstance((int) id, blockPosition,
					breakAmount);
			sendPacket(player, packet);
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public static boolean setBlockFast(Chunk chunk, int x, int y, int z, Material material, int data) {
		if (class_Block_fromLegacyData == null || class_CraftMagicNumbers_getBlockMethod == null
				|| class_Chunk_setBlockMethod == null || class_BlockPosition_Constructor == null) {
	...
	public static boolean setPickupStatus(Arrow arrow, String pickupStatus) {
		if (arrow == null || pickupStatus == null || class_Arrow_setPickupStatusMethod == null
	...
	public static Entity getEntity(World world, UUID uuid) {
		try {
			Object worldHandle = getHandle(world);
			final Map<UUID, Entity> entityMap = (Map<UUID, Entity>) class_WorldServer_entitiesByUUIDField
					.get(worldHandle);
			if (entityMap != null) {
	...
	public static Material fromLegacy(org.bukkit.material.MaterialData materialData) {
		if (class_UnsafeValues_fromLegacyDataMethod != null) {
			try {
				Material converted = (Material) class_UnsafeValues_fromLegacyDataMethod
						.invoke(DeprecatedUtils.getUnsafe(), materialData);
				if (converted == Material.AIR) {
					materialData.setData((byte) 0);
					converted = (Material) class_UnsafeValues_fromLegacyDataMethod.invoke(DeprecatedUtils.getUnsafe(),
							materialData);
				}
				// Converting legacy signs doesn't seem to work
				// This fixes them, but the direction is wrong, and restoring text causes
				// internal errors
				// So I guess it's best to just let signs be broken for now.
				/*
				 * if (converted == Material.AIR) { String typeKey =
				 * materialData.getItemType().name(); if (typeKey.equals("LEGACY_WALL_SIGN"))
				 * return Material.WALL_SIGN; if (typeKey.equals("LEGACY_SIGN_POST")) return
				 * Material.SIGN_POST; if (typeKey.equals("LEGACY_SIGN")) return Material.SIGN;
				 * }
				 */
				return converted;
			} catch (Exception ex) {
	...
	public static String migrateMaterial(String materialKey) {
		...
		Material legacyMaterial = data == 0 ? getLegacyMaterial(materialName)
				: Material.getMaterial("LEGACY_" + materialName);
	...
	public static boolean setParrotVariant(Entity entity, Enum<?> variant) {
		if (class_Parrot == null || variant == null)
			return false;
		try {
	...
	public static String getBlockData(Material material, byte data) {
		if (class_UnsafeValues_fromLegacyMethod == null)
			return null;
		try {
	...
	public static String getBlockData(Block block) {
		if (class_Block_getBlockDataMethod == null)
			return null;
		try {
	...
	public static boolean applyPhysics(Block block) {
		if (class_World_setTypeAndDataMethod == null || class_World_getTypeMethod == null
				|| class_BlockPosition_Constructor == null)
			return false;
		try {
			Object worldHandle = getHandle(block.getWorld());
			Object blockLocation = class_BlockPosition_Constructor.newInstance(block.getX(), block.getY(),
					block.getZ());
	...
	public static boolean addRecipeToBook(ItemStack book, Plugin plugin, String recipeKey) {
		if (class_NamespacedKey_constructor == null || class_KnowledgeBookMeta_addRecipeMethod == null)
			return false;
		ItemMeta meta = book.getItemMeta();
		if (!class_KnowledgeBookMeta.isAssignableFrom(meta.getClass()))
			return false;
		try {
	...
	public static boolean setTopHalf(Block block) {
		...
		try {
			Object blockData = class_Block_getBlockDataMethod.invoke(block);
			if (blockData == null || !class_Bisected.isAssignableFrom(blockData.getClass()))
				return false;
