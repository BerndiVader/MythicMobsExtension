...
public class Pre44MobSpawnEvent implements Listener {
	...
	public Pre44MobSpawnEvent() {
		...
		for (int i1 = 0; i1 < listeners.length; i1++) {
			...
			if (listener.getClass().getName()
					.equals("io.lumine.xikage.mythicmobs.adapters.bukkit.listeners.MobListeners")) {
	...
	public void MobSpawnEvent(CreatureSpawnEvent e) {
		...
		if (mythicmobs.getCompatibility().getWorldGuard().isPresent()
				&& !mythicmobs.getCompatibility().getWorldGuard().get().LocationAllowsMobSpawning(e.getLocation())) {
			return;
		}
		RandomSpawnPoint rsp = new RandomSpawnPoint(BukkitAdapter.adapt((Entity) e.getEntity()),
				BukkitAdapter.adapt(e.getLocation()), e.getSpawnReason());
				...
				Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin) mythicmobs, new Runnable() {
					...
					public void run() {
						if (!Utils.mobmanager.isActiveMob(spawn.getUniqueId())) {
							MythicMobs.debug(3,
									"Compatibility mode enabled and found custom mob spawn! Checking for MythicMob '"
											+ l.getCustomName() + "'...");
