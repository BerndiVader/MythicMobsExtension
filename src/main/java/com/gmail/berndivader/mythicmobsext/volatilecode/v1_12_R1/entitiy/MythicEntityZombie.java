...
public class MythicEntityZombie extends EntityMonster {

	protected static final IAttribute a = new AttributeRanged(null, "zombie.spawnReinforcements", 0.0, 0.0, 1.0)
	...
	public void n() {
		float f2;
		if (this.world.D() && !this.world.isClientSide && !this.isBaby() && this.sunBurn && this.p()
				&& (f2 = this.aw()) > 0.5f && this.random.nextFloat() * 30.0f < (f2 - 0.4f) * 2.0f
				&& this.world.h(new BlockPosition(this.locX, this.locY + (double) this.getHeadHeight(), this.locZ))) {
			boolean flag = true;
	...
	public boolean damageEntity(DamageSource damagesource, float f2) {
		if (super.damageEntity(damagesource, f2)) {
			...
			if (entityliving != null && this.world.getDifficulty() == EnumDifficulty.HARD
					&& (double) this.random.nextFloat() < this.getAttributeInstance(a).getValue()
				...
				while (l < 50) {
					...
					if (this.world.getType(new BlockPosition(i1,
							(j1 = j + MathHelper.nextInt(this.random, 7, 40) * MathHelper.nextInt(this.random, -1, 1))
									- 1,
							k1 = k + MathHelper.nextInt(this.random, 7, 40) * MathHelper.nextInt(this.random, -1, 1)))
							.q() && this.world.getLightLevel(new BlockPosition(i1, j1, k1)) < 10) {
						entityzombie.setPosition(i1, j1, k1);
						if (!this.world.isPlayerNearby(i1, j1, k1, 7.0)
							...
							this.getAttributeInstance(a).b(new AttributeModifier("Zombie reinforcement caller charge",
									-0.05000000074505806, 0));
							entityzombie.getAttributeInstance(a).b(new AttributeModifier(
									"Zombie reinforcement callee charge", -0.05000000074505806, 0));
							break;
						}
					}
					++l;
		...
		if (flag) {
			...
			if (this.getItemInMainHand().isEmpty() && this.isBurning() && this.random.nextFloat() < f2 * 0.3f) {
				EntityCombustByEntityEvent event = new EntityCombustByEntityEvent(this.getBukkitEntity(),
						entity.getBukkitEntity(), 2 * (int) f2);
				this.world.getServer().getPluginManager().callEvent(event);
				if (!event.isCancelled()) {
	...
	public void b(EntityLiving entityliving) {
		...
		if ((this.world.getDifficulty() == EnumDifficulty.NORMAL || this.world.getDifficulty() == EnumDifficulty.HARD)
			...
			entityzombievillager.prepare(this.world.D(new BlockPosition(entityzombievillager)),
					new GroupDataZombie(false, null));
	...
	public GroupDataEntity prepare(DifficultyDamageScaler difficultydamagescaler,
		...
		if (object instanceof GroupDataZombie) {
			...
			if (entityzombie_groupdatazombie.a) {
				...
				if ((double) this.world.random.nextFloat() < 0.05) {
					List<EntityChicken> list = this.world.a(EntityChicken.class,
							this.getBoundingBox().grow(5.0, 3.0, 5.0), IEntitySelector.b);
					if (!list.isEmpty()) {
		...
		if (this.getEquipment(EnumItemSlot.HEAD).isEmpty() && (calendar = this.world.ae()).get(2) + 1 == 10
				&& calendar.get(5) == 31 && this.random.nextFloat() < 0.25f) {
			this.setSlot(EnumItemSlot.HEAD,
		...
		this.getAttributeInstance(GenericAttributes.c)
				.b(new AttributeModifier("Random spawn bonus", this.random.nextDouble() * 0.05000000074505806, 0));
		...
		if (this.random.nextFloat() < f2 * 0.05f) {
			this.getAttributeInstance(a)
					.b(new AttributeModifier("Leader zombie bonus", this.random.nextDouble() * 0.25 + 0.5, 0));
			this.getAttributeInstance(GenericAttributes.maxHealth)
					.b(new AttributeModifier("Leader zombie bonus", this.random.nextDouble() * 3.0 + 1.0, 2));
			this.p(true);
		}
		return object;
	}
	...
	public void die(DamageSource damagesource) {
		...
		if (damagesource.getEntity() instanceof EntityCreeper
				&& (entitycreeper = (EntityCreeper) damagesource.getEntity()).isPowered()
				&& entitycreeper.canCauseHeadDrop()) {
	...
	class PathfinderGoalAttack extends PathfinderGoalMeleeAttack {
