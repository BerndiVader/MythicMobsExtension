...
public class MythicEntityParrot extends EntityParrot {
	private static final Set<Item> bJ = Sets.newHashSet(Items.WHEAT_SEEDS, Items.MELON_SEEDS, Items.PUMPKIN_SEEDS,
	...
	public boolean a(EntityHuman eH, EnumHand eh) {
		...
		if (!this.isTamed() && bJ.contains(itemstack.getItem())) {
			if (!eH.abilities.canInstantlyBuild)
				itemstack.subtract(1);
			if (!this.isSilent())
				this.world.a(null, this.locX, this.locY, this.locZ, SoundEffects.eJ, this.bK(), 1.0f,
		...
		if (itemstack.getItem() == Items.COOKIE) {
			...
			new BukkitRunnable() {
				@Override
				public void run() {
					((EntityPlayer) eH).playerConnection
							.sendPacket(new PacketPlayOutSetSlot(0, 36 + eH.inventory.itemInHandIndex, itemstack));
				}
